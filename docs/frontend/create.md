...
vue 做的主要工作是将我们用 template 模板或者 h 函数创建的视图插入到页面相应的位置，在数据更新时通知视图更新
...
function createApp(...args) {
    ...
    const { mount } = app
    app.mount = (container) => {
        container = typeof container === 'string' ? querySelector(container) : container
        container.innerHTML = ''
...
2. createApp 创建应用实例，接收根组件、根属性作，返回的实例包括 mixin、use、mount 等方法
...
当调用 mount 时开始了组件的挂载流程，我们根据根组件创建 vnode，接收的 rootComponent 可能为组件、函数组件或字符串，返回的 vnode 是一个包含节点类型、属性、子节点等属性的对象。
...
假设上面我们的 rootComponent 是一个组件，则在此阶段调用 processComponent 进行挂载
...
在执行继续之前，先介绍下 h 函数，它通过对 createVNode 的封装让我们更方便的创建 vnode

在组件的 render 方法内，我们通常调用 h 函数来创建组件最终需要渲染的内容
...
对组件的挂载主要在 mountComponent 方法中进行，主要做下面几件事：
...
在 setupRenderEffect 中，执行 instance.render 得到子树，假设此时我们在 render 内返回 h 函数创建的元素节点 `h('div', { style: {color: 'red'}}, 'hello')` 后文用 ele 指代它
...
注意当 children 是数组（ `['hello', 'word']` ）时，如果对每个都当作 TEXT_CHILDREN 处理，则后添加的 'word' 就会覆盖之前的 'hello'，所以，在此处创建一个虚拟的 Text 节点，将子元素作为虚拟节点的孩子处理

## 6. 更新流程

上面我们将 componentUpdateFn 放入了 effect，这样当用户组件的 render 内使用的数据改变时，会重新执行
...
当对元素进行更新时，我们需要更新它的属性和儿子

+ 对于属性更新，我们直接删除老属性中不存在的属性
...
+ 对于孩子的更新，children 有 3 种类型，分别是 text、array 或者没有（null），我们用 oldChildren 代表老的孩子，newChildren 代表新的孩子

    1. newChildren 是 text，那么设置 parent 的 textContent 为 newChildren 即可

    2. 如果 oldChildren 不是 array，同样挂载 newChildren 到 parent 即可

    3. 如果 newChildren 和 oldChildren 都是 array，说明会有可以复用的节点，进行 diff 比较
...
+ 为了复用节点，我们从两边开始比较
    ...
    2. 找完后 oldChildren 有剩余，需要删除；否则 newChildren 有剩余，需要挂载

    3. 剩下的节点对应位置的元素都不相同，说明发生了位置移动或者元素替换，用 toBePatched 代表需要操作的节点个数
        
        - 首先我们找到 oldChildren 和 newChildren 相同的节点进行复用
        ...
        - 根据 toBePatched 从后往前查找 newChildren
            ...
            - 如果是复用过的节点 nodeI，我们尝试得到 newIndexToOldIndexMap 的最长递增子序列相对应序号构成的数组 increaseArr，由于我们是从后往前查找，如果 nodeI 对应的序号在 increaseArr 中，说明跳过它即可，否则，需要插入它
